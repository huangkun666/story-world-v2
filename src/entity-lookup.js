...
// 按需查书补字段（**实力**；位置自 leg25 f 起改由零 token 的结构推断供给，见下方 ENTITY_LOOKUP_FIELDS 注释）
// ——编排层纯函数 + 注入式模型调用。
...
//   ①**零 token 的结构推断已经吃到位置**（`deriveLocationFromBook`：组织条目驻地 → 成员，真账实测 173 个，
//     幂等、加载期自动跑）——位置的第一供给面是**结构**，不是模型；
//   ②剩下没推出来的那批，书里**确实没有地名**：实测 `幽都` 的条目驻地原文是
//     「轮回破碎后亿万魂魄滞留所建」（描述句，归一化拿不到位置集内地名）⇒ 送进模型**也是白烧**；
//   ③这栏的判据本来就是**错位的**：`fields` 里的中文键 `'位置'` 查的是 `entity['位置']`，而账本用的是
//     英文键 `location`（实测：中文键 0/563、`location` 有占位值 563/563）——两条路口径不一致，
//     既查不准，又让"占位值算不算有值"这个伪问题长期挂在名单上。
//   ⇒ 与硬规矩「空着就是空着」一致：**没有可靠供给面的字段，不进查书队列**。
//   `applyLookup` 里那段"位置并入 location"的归一化代码保持不动（旧账留档与 `位置来源:'书里原话'`
//   仍可读，且它是 `normalizeToPositionSet` 的测试面）；只是本模块不再**主动**去问模型要位置。
export const ENTITY_LOOKUP_FIELDS = ['实力'];   // 本模块的面（势力不抽实力——用户拍板；位置改由结构推断供给）
