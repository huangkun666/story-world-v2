...
// 语义：静默 = 不主动（actions/盘算推进/plot 事件），不是消失——静默方始终是合法客体；
...
export function gateWorldStep(step, world, moveFact = null) {
    ...
    for (const ev of world.events || []) {
        if (!ev.closed) for (const r of ev.ripples || []) named.add(r);
    }
    for (const a of step.actions || []) if (a.target) named.add(a.target);
    if (moveFact?.object) named.add(moveFact.object);
    ...
    const dropped = { actions: [], agendaAdvances: [], plotEvents: [], newAgendas: [], agendaCancels: [] };
    ...
    const droppedCounts = {};
    for (const id of [...dropped.actions, ...dropped.agendaAdvances, ...dropped.plotEvents, ...dropped.newAgendas, ...dropped.agendaCancels]) {
        droppedCounts[id] = (droppedCounts[id] ?? 0) + 1;
    }

    return {
        step: { actions, newEvents, agendaAdvances, stateChanges: step.stateChanges || [], newAgendas, agendaCancels },
        silent: [...silentSet],
        lifted,
        dropped,
        droppedCounts,
