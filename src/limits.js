...
export const ENTITY_BIRTH_PER_TICK = 1;  // settle.js：一轮最多入局几个新人（★leg63 起**给旋钮**了）
export const IDLE_FACES_TOP = 12;    // pack.js：一轮给几个冷门人点名机会（★leg63 起**给旋钮**了；**不影响名册**）
...
export const LIMIT_DEFAULTS = Object.freeze({
    ...
    // ★★★leg63（用户令「**我要把另外两个参数也设置成可调**」）：最后两个"不给拧"的转正。
    //   它们过去是丙档只读，理由写在各自的位置（`pack.js:593`"一次调太多，每个都得重跑基线"）——
    //   而真账里这两个数**会咬人**：
    //     · `每轮入局`（出厂 1）：模型一轮提 2 个新人 ⇒ 第 2 个被拒（世界永远只能一轮进一个）。
    //     · `待启用名单`（出厂 12）：它是"给长期没露面的人一次起头资格"的那份名单
    //       （`settle.js` 把同一份交给门控）⇒ **名单多长，一轮里最多就有几个冷门人能开口**。
    每轮入局: ENTITY_BIRTH_PER_TICK,     // settle.js：一轮最多入局几个新人（超出的按"入局限额"拒并留痕）
    待启用名单: IDLE_FACES_TOP,          // pack.js：一轮给几个冷门人点名机会（同时决定门控放行几个；**名册不受它影响**）
 ...
 *  ★★★leg63 格式口径（本棒被自己的判据抓了三次，定死免得下一棒再踩）：
 *    **这几段 hint 是纯文本，不许夹任何标记**——它们经 `render.js` 的 `escapeHtml(r.meta.hint)` 上板，
 *    写 `<b>` 会**原样印成 `&lt;b&gt;`**（字面标签，比不强调更坏）；写 markdown `**` 会印出星号。
 *    要强调就用中文引号「」（本仓既有写法），或者干脆不强调。
 *  ★为什么不能改成"不转义"：那等于在面板上开一个"这段字符串受信任"的口子——
 *    而这些文案将来会被复制到别处（信息带/自检卡），口子会跟着走。 */
export const LIMIT_META = Object.freeze({
    每轮递线: {
        label: '每轮递几条线',
        hint: '每轮自动把"开了头还没人接的线"递到模型眼前，要求每条各写一步。'
            + '调高 ⇒ 一轮里同时推进的线更多（实测 3 → 每轮 2.25 条、6 → 6.00 条）；'
            + '代价是一轮要写的事更多，且「同时最多几件大计」会先见底。',
    },
    每轮事件: {
        label: '每轮最多几件事件',
        hint: '一轮里最多记下几件新事，超出的被拒（面板会如实报"拒了几件"）。'
    ...
    每轮新生: {
        label: '每轮最多新起几件大计',
        hint: '一轮里最多「新开」几件大计，超出的提议当轮被拒（观棋窗口底部会如实报"拒了几件"）。'
            + '现行 3：这是三道盘算上限里「最先」判的一道——它顶住了，下面「同时最多几件大计」'
            + '与「同时在办总数」就都还没轮到判。'
            + '想让一轮里多长几件事，这一格与「每轮递几条线」要「一起」抬。',
    },
    // ★★★leg63：最后两个转正的旋钮（用户令「我要把另外两个参数也设置成可调」）。
    //   它们过去是丙档只读（理由在各自的位置：`pack.js`"一次调太多，每个都得重跑基线"），
    //   而真账里这两个数**会咬人**：
    //     · `每轮入局`（出厂 1）：模型一轮提 2 个新人 ⇒ 第 2 个也被拒（世界只能一轮进一个）。
    //     · `待启用名单`（出厂 12）：它是"给长期没露面的人一次起头资格"的那份名单
    //       （`settle.js` 把同一份交给门控）⇒ **名单多长，一轮里最多就有几个冷门人能开口**。
    每轮入局: {
        label: '每轮最多入局几个新人',
        hint: '一轮里最多让几个新面孔进这个世界（超出的提议当轮被拒，观棋窗口会如实报）。'
            + '现行 1：模型一轮提了两个人，第二个也会被拒——世界只能一轮进一个。',
    },
    // ★★★leg63（用户当场质疑「人物不是每轮都递的所有吗？」）：**这一格原来写错了**。
    //   错的写法：「引擎每轮机械挑一批**从没出过手**、也没人点名的人，递到模型眼前」。
    //   实际口径（`pack.js` 的 `computeIdleFaces`，六个条件同时成立）：
    //     active ∧ 手上没有在办盘算 ∧ `lastActiveTick` 距今 **≥ QUIET_TICKS(12) 轮**（或从没出过手）
    //     ∧ 不在未决事件波及里 ∧ 不是玩家 ∧ 不是 top-1 保送
    //   ⇒ ① **不是"从没出过手"**（沉默 ≥12 轮的也算），② **不是"递到模型眼前"的唯一那份名单**
    //      （**全部在册实体每轮都在包的实体表里**，这一份是**额外**的"点名候选"）。
    //   为什么必须写准：用户就是照这句问的「这是啥意思，人物不是每轮都递的所有吗？」
    //   —— 一句不准的 hint 会让人以为"人物要靠这份名单才进得了包"（那会让他去拧它来"让全世界都出现"）。
    待启用名单: {
        label: '每轮给几个"冷门人"点名机会',
        hint: '这只是「全部在册人物的名册之外」额外递的一份小名单（名册本身每轮都在，不受这一格影响）。'
            + '名单上的人是每轮自动挑的：还在册、手上没有在办的事、并且已经沉默了 12 轮以上'
            + '（或从没出手过）、也没被哪件未了的事牵扯——就是那些"世界一直没轮到他们"的人。'
            + '每轮轮着换一批，被点到或出过手的人下一轮就有资格自己立线。'
            + '调高 ⇒ 一轮里能轮到的冷门人更多（世界更不围着那几家转）；代价是一轮要写的事更多，'
            + '且他们立起的线要各占「同时最多几件大计」的名额。现行 12。',
    },
    顶层大计: {
        label: '同时最多几件大计',
