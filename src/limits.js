...
export const AGENDA_CAP_PER_TICK = 3;// settle.js：一轮最多新起几件盘算（★leg63 起**给旋钮**了，见下）
...
export const LIMIT_DEFAULTS = Object.freeze({
    ...
    // ★★★leg63（用户实机报「我参数都这样了」——把「每轮递几条线」拧到 10，仍被
    //   `盘算大厦顶（每 tick 新生 ≤3）` 拒签）：这一格**过去没有旋钮**，而出厂值 3 恰好是
    //   "一轮最多新起几件盘算"的那道闸（`settle.js` 的 `spawnAgendas` 第一道 if）。
    //   为什么现在必须给它旋钮（用户原话：「就和这四个参数一样就行，不要挡位，就直接输入框」）：
    //     · 三道盘算闸是**按序判**的，这道在最前 ⇒ 它一旦咬住（3），后面 `在飞大计`/`顶层大计`
    //       连被检查的机会都没有 ⇒ 玩家把「每轮递线」拧到 10 的意图**被一个他看不到的数吃掉**；
    //     · 旧口径的理由是"与每轮事件撞在一起调会互相掩盖"——而**掩盖的成因正是它不可调**：
    //       一个不给拧的闸，只会让别的旋钮看起来没生效（正是用户这次遇到的）。
    每轮新生: AGENDA_CAP_PER_TICK,      // settle.js：一轮最多新起几件盘算（超出的提议按"盘算大厦顶"拒并留痕）
...
export const LIMIT_META = Object.freeze({
    每轮递线: {
        ...
        hint: '引擎每轮把"开了头还没人接的线"递到模型眼前，要求每条各写一步。'
            ...
            + '代价是一轮要写的事更多，且<b>同时最多几件大计</b>会先见底。',
    },
    每轮事件: {
        label: '每轮最多几件事件',
    ...
    // ★★★leg63：第五个旋钮（来路见 `LIMIT_DEFAULTS` 里那一段）。
    //   ★措辞纪律（A-3 玩家可见文本零引擎术语）：不许出现"闸/perTick/盘算大厦顶"这类词；
    //     "盘算"这个词面板别处已经在用（观棋窗口那三格），故这里沿用"大计"。
    //   ★★格式纪律（**本棒被自己的判据抓了两次**）：这四段 hint 是**直接拼进 HTML** 的
    //     （`render.js` 的 `escapeHtml(r.meta.hint)`）⇒ 里面只能用 `<b>`，**不许写 markdown 的 `**`**
    //     ——写了就会在页面上原样显示两个星号（leg60 为这条栽过，本棒又栽两次）。
    每轮新生: {
        label: '每轮最多新起几件大计',
        hint: '一轮里最多<b>新开</b>几件大计，超出的提议当轮被拒（观棋窗口底部会如实报"拒了几件"）。'
            + '现行 3：这是三道盘算上限里<b>最先</b>判的一道——它顶住了，下面"同时最多几件大计"'
            + '与"同时在办总数"就都还没轮到判。'
            + '想让一轮里多长几件事，这一格与「每轮递几条线」要<b>一起</b>抬。',
    },
    顶层大计: {
        label: '同时最多几件大计',
