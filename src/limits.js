...
export const IDLE_FACES_TOP = 12;    // pack.js：待启用名单每轮递几张脸（★leg63 起**给旋钮**了）
...
export const LIMIT_DEFAULTS = Object.freeze({
    ...
    //     · `待启用名单`（出厂 12）：它是"给从没出过手的人一次起头资格"的那份名单
    //       （`settle.js` 把同一份交给门控）⇒ **名单多长，一轮里最多就有几个局外人能开口**。
    每轮入局: ENTITY_BIRTH_PER_TICK,     // settle.js：一轮最多入局几个新人（超出的按"入局限额"拒并留痕）
    待启用名单: IDLE_FACES_TOP,          // pack.js：一轮往模型眼前递几张"从没出过手"的脸（同时决定门控放行几个）
...
export const LIMIT_META = Object.freeze({
    ...
    待启用名单: {
        label: '每轮递几张"还没出过手"的脸',
        hint: '引擎每轮机械挑一批「从没出过手、也没人点名」的人，递到模型眼前，并给他们一次起头资格。'
            + '调高 ⇒ 一轮里能有更多局外人开口（世界更热闹）；代价是一轮要写的事更多，'
            + '且这些线要各占「同时最多几件大计」的名额。现行 12。',
    },
    顶层大计: {
        label: '同时最多几件大计',
