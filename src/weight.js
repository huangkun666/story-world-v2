...
// leg29（用户 2026-09-12 明示，当时令「别做了，交接吧」⇒ leg28 未实施）：「事件波及也改成 15 个」⇒ 3 → 15。
//   ★实测（真 checkWorldStep，改前跑）：本上限**不是**实际卡人的那道闸——`checkAgendaInvolvement` 的
//     「单盘算一轮涉及 ≤15」把属主 + **本步全部** actions 的 entity/target + 波及名单**并成一个集合**，
//     而波及名单是它的子集 ⇒ 有效天花板 = 15 −（属主与行动方去重后的个数）：
//     属主自行动 → 最多波及 **14**；属主 + 1 个行动方 → **13**；+3 个 → **12**；+5 个 → **10**。
//     ⇒ 取 15 后两道闸的边界重合：**15 个波及只有在波及名单把本步全部动作方都包含进去时才可能过**，
//       否则先被涉及闸拒。故「写进提示词的上限」必须带上这条咬合（prompts.js 铁律 8 已写），
//       否则模型会照着"至多 15 个"写出 16 个 ⇒ 拒整步 ⇒ 白烧一轮（正是告知面要防的那件事）。
//   ★副作用（只陈述事实，未在本次处置）：波及名单进两处——①`pack.js:34` 的 `named` 集合
//     （未决事件 ripples）⇒ 镜头第①段"保送"，且同一集合正是 `gate.js` 的"被点名可应答"例外：
//     这一半**与"世界变宽"同向**（更多静默者获得镜头位与出手权）；②`checkAgendaInvolvement`
//     把波及名单计入盘算涉及面 ⇒ 波及越宽越容易撞 15 的涉及闸（另一半与变宽反向）。
//     净效应未测，不臆断；`docs/spec-world-widening.md` §2 的集中度读数（前 3 名占 81%）是改前基线。
export const RIPPLE_TARGET_CAP = 15;
