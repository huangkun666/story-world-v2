...
// 失败零阻塞：调用抛错/超时/非法 JSON/非对象 → parsed=null → **什么都不写**（leg24 审计处置：
// 定案默认值已作废——解析不出依据的维度就空着，引擎不替玩家编数）。
...
export function buildPlayerParsePrompt(playerDesc) {
    return [
        '你是玩家开档描述的抽取器。只提取不创作：从下面的描述里摘出玩家的姓名与实力陈述，没有依据的一律不输出。',
        '输出严格 JSON，形状如下（name 无依据就不写；四维全可选，数值域 [0,1]；可省字段不写 null）：',
        JSON.stringify({ name: '黄坤', hardPower: 0.5, office: 0.5, intel: 0.5, network: 0.5 }, null, 2),
        'name = 玩家姓名（描述里没写就不输出）｜hardPower = 武力/修为 ｜ office = 权位/职务 ｜ intel = 耳目/知晓 ｜ network = 人脉/门路',
        '———— 玩家开档描述如下 ————',
        playerDesc,
    ].join('\n');
}
