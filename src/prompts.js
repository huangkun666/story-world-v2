...
7. 可以提议新盘算（newAgendas），必须带来源：event=由某未决事件而生（ref=该事件 id）/ parent=由某在飞盘算委派而生（ref=该盘算 id）/ state=由世界处境而生（不带 ref）。无源之物不存在。盘算的出生、上限与结算全归引擎裁决，你只有提议与推进权。也可以提议放弃盘算（agendaCancels，agendaId 必填 + 理由）——放弃同样是提议，终结与否全归引擎裁决，你只有提议权。可以提议新实体入局（newEntities，name/kind/location + 源：book=书名录条目（ref=条目名）/ event=某未决事件（ref=事件 id）/ dialogueFact=对话依据册中反复被点名的对象（ref=对象名））——出生、单轮上限、席位上限全归引擎裁决。可以提议实体覆灭（entityFates，entity + verdict=dead + 源：event/agenda + ref + 理由）——覆灭与否全归引擎复核（引用必须真实落账、目标无在飞盘算、玩家不可灭），你只有提议权；打崩不是灭，覆灭由引擎裁定。
