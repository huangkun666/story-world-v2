...
export function renderParamsHtml(world, { config = {} } = {}) {
    ...
    const capCard = `<div class="sw2-set-card sw2-cap-card" style="grid-column:1/-1">`
        ...
        + foldHint('这一栏决定<b>这个世界允许跑多宽</b>；改完<b>下一轮生效</b>（已落账的账不回改）。'
            ...
            + '<br>★实机例子（leg63 用户报的）：把「每轮递几条线」抬到 10、模型真提了 10 条，'
        ...
        + foldHint('',
            ...
            { summary: '每个上限各是什么意思' })
        + foldHint('',
            // ★★★leg63：本条原来列三项（含"每轮新生大计 ≤3"）——那一条**已经给了旋钮**（第五个输入框），
            //   故从"不给拧"这一栏撤下。"与上面几个撞在一起调会互相掩盖"这个理由也随之不成立：
            //   掩盖的成因正是"有个闸不给拧"（见 `limits.js` 的 `每轮新生` 那一段）。
            `仍然固定（不给旋钮）：<b>每轮入局新人 ≤${ENTITY_BIRTH_PER_TICK}</b> · `
            + `<b>每轮递几张待启用名单 ${IDLE_FACES_TOP}</b>`
