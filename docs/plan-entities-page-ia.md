...
test('细案实体页：排序三档（在办优先 / 最近活跃优先 / 按名号）', () => {
    ...
    //   ★★本节留档两处我写错的东西，都在这里改正（诚实留档比好看重要）：
    //   ① **期望值本身**：初稿写 `['丙','乙','甲']`，那既不是拼音序、也不是笔画序（实测 `zh-u-co-stroke` 给 `乙丙甲`），**就是错的**。
    //   ② ★★★**我由此推出的"ICU 无拼音排序数据"是错的**（错了两轮才查清）。**实测铁证**：
    //      `localeCompare(name, 'zh')` **就是拼音序**——取真账 621 名号里首字有拼音参照的 **274 个**、
    //      做 **36629 个可比对**，按拼音参照比，**逆序对 = 0（逆序率 0.00%）**。
    //      三重错：**(a)** 从 `Intl.Collator('zh-u-co-pinyin').resolvedOptions().collation === 'default'` 推出
    //      "没有拼音数据"——**不成立的推断**（`'default'` 是"默认排序规则"的名字，不等于退化成码点序）；
    //      **(b)** 判别样本 `[丙,甲,乙]` **恰好两种规则下同序**，根本判别不了；
    //      **(c)** 把末尾「祝无双 · 转轮鬼圣 · 转轮鬼使 · 转轮王 · 追风 · 坐忘大罗」当成"被拆散"——
    //      **它们恰恰是拼音序**：祝 zhù < 转 zhuǎn < 追 zhuī < 坐 zuò。
    //      ⇒ **本页"按名号"排序没有问题，产品无此限制**；这条曾被写进细案/交接/台账，现已全部改正。
    assert.deepEqual(selectEntityPage(w, { ...base, sort: 'name' }).rows.map((e) => e.name), ['丙', '甲', '乙'], '按名号（ICU 中文序 = 拼音序）');
});
...
| **★ Task 1 评审的 Minor 定夺（已写回计划）** | ① **排序期望值我写错了**：`sort:'name'` 的正确期望是 `['丙','甲','乙']`（丙 bǐng < 甲 jiǎ < 乙 yǐ），初稿写的 `['丙','乙','甲']` 既非拼音序也非笔画序 ⇒ 已改正。★★★**而由它推出的"ICU 78.3 无拼音排序数据"更是错的**（错了两轮才查清）：实测 `localeCompare(name,'zh')` **就是拼音序**——真账 274 个有名参照的名号、**36629 个可比对、逆序对 0（0.00%）**。三重错：从 `collation === 'default'` 推出"无拼音数据"（不成立的推断）· 判别样本 `[丙,甲,乙]` 恰好两规则同序（判别不了）· 把「祝无双·转轮鬼圣·转轮鬼使·转轮王·追风·坐忘大罗」当"被拆散"（**那恰恰是拼音序**：zhù < zhuǎn < zhuī < zuò）⇒ **"按名号"排序无问题，产品无此限制**；错话已从细案/交接/台账/计划全部撤除。② 空结果 `from/to` **按意图都是 0**，分页器不印「显示第 0–0 条」。③ `entsSearchTextOf` 不再收「未明」（占位词不是内容，否则搜「未明」命中 475 人）。④ `recent/named/orphan` 两个调用点各写一遍谓词——评审说现在**过早**（只有两处），**留到 Task 3 接 chip 时若出现第三处消费者再提成一张表**。 |
