...
判据给用户：**硬刷新（Ctrl+Shift+R）后页脚应显示 `构建 leg66-cause-at-batch-entry`**
...
1. **硬刷新后面板打得开吗**、页脚构建号是不是 `leg66-cause-at-batch-entry`；
...
| `PANEL_BUILD` | **`leg66-cause-at-batch-entry`**（收尾时因"裁定文案"那一格升位，见 §2.4；UI 版式本身一个字没动） | 本棒 |
