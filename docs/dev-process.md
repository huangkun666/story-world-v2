...
- **测试**：`node --test`（**必须无参**——Node 24 下目录参数会被当模块加载）。当前 216/216 全绿（2026-09-07 第八棒实测复核）。
...
| 大势层数字正式报批（熵泵节奏/键表/带值/漂移步 + 强度算法参数 + TIDE_CAP + 注入词表） | **二批包在案**（`docs/ratification-batch-b2-2026-09-07.md`，16 项数值+证据索引） | 用户点头报批二批 | 100t 曲线：熵泵种子 ≥8、强度域 [0,1] 多变且冷档衰减、挂因 ≥2、预算内零警告（台账 L70-L73） |
