...
> **接手第一入口 = `docs/START-HERE.md`（一页，≈5 KB，读完即可开工）。**
> 2026-09-11「文档重塑」立的规矩：**接手不许预读全库**。改前实测：接手清单 6 份共 **379 KB**
> （光台账全文就 226.9 KB、ANCHOR 状态行单行 9.4 KB），读完成本 ≈4.5 万 token ——
> 一棒还没动手就先烧掉三成上下文，这就是"每棒都感觉刚搞清楚在干嘛就得交接"的根因。
> 现法：一页入口 + 索引 + 全文按需查；**任何文件都不许再长成"必须通读"的形态**。

| 文档 | 职责（性质） | 何时建 | 何时更新 | 谁拍板 |
|---|---|---|---|---|
| `docs/START-HERE.md` | **接手第一入口（唯一）**：走到哪 + 三条不可越的线 + 动手前四条规矩 + 按需查表 + 现场 | 文档重塑（2026-09-11） | 每棒交接时更新**且保持短**（≈5 KB 上限）；长了就往 `LEDGER.md`/`docs/` 挪 | 用户 |
| `LEDGER.md`（项目根） | **台账索引**：一行 = 一次变更（日期/谁/一句话），共 174 行 ≈12 KB；细节指针去 `docs/ledger.md` | 文档重塑（2026-09-11） | 与全文**同批**各加一行（两处都加） | 记录即可，无拍板 |
| `ANCHOR.md` | **唯一尺子**：方向、架构轮廓、已定规则、未决点、决策记录 | 开工前建（v2 由零代码的第一棒产出） | 状态行随阶段回填——**只写"当前棒 + 下一动作 + 指针"，不许再累积历史长文**（2026-09-11 前它曾长到单行 9.4 KB，已收进交接/台账）；机制级变更走细案 | 用户 |
| `docs/*-spec.md` | 机制细案：设计 + 步骤 + 验收判据 + 范围限制 | 机制新增时建（两段式第一步） | 拍板时定稿；验收通过后状态行更新；实施偏差记台账    | 用户 |
| `docs/ledger.md` | 工程台账**全文**：变更与决策实录（改码三件事之三） | 首个变更时建 | **每次改码/决策/实测补一行**；验收/交接也记（同时在 `LEDGER.md` 加索引行） | 记录即可，无拍板 |
| `docs/handoffs/session-handoff-*.md` | 交接：现状盘点 + 下一棒任务书 | 阶段/棒次结束时建 | 每棒一份，接力 | 用户（任务书内容） |
| `docs/dev-process.md` | 本手册：流程怎么走、文档管什么、问题停在哪 | 第三棒建 | 交接棒发现手册与现状脱节时更新；**新增纪律须用户拍板** | 用户 |
| `docs/handoffs/` | **归档区**：历史交接（leg1–leg25；当前交接留 `docs/` 根作入口） | 知识库实施（2026-09-10） | 交接归档时移入并全局更新引用路径 | 用户 |
...
| 最新交接 §4 | **待办事项总账**（A 待拍板/B 待执行/C 现场事务/D 已结，全带触发条件） | leg21 起（2026-09-10） | 每棒交接时同步整理；被点即拉出执行 | 用户 |
...
| 文档知识库（docs 归档 handoffs/archive + §2 文档状态表 + decision-index 决策速查 + 细案生命周期收敛） | **待办（2026-09-09 用户令：仅登记不实施）** | 用户指令「做知识库」 | 归档迁移清单（含 docs/ 根现存文件逐份判定） |
