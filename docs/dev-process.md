...
- **已入队的补强**（本棒提出、未实施、走 §6 流转）：
  ...
  ② **事件每 tick 产率上限**——新生盘算有 ≤2/回合，事件没有上限（旁观演示波及链双线并进、一轮 18+ 条）；"死"半缺闸，数值待曲线（提案方向 ≤10/回合）。
...
| GC 数字正式报批 | 曲线在手 | 分量引擎细案拍板前完成 | 冒烟曲线（≤2 新生/≤15 在飞/≤5 宏大、输入峰 199/4000t） |
| 设定注入缺口（未决点 4） | **细案草稿在案**（`docs/backdrop-layer-spec.md`，2026-09-07 夜：备料完成 + 形状草案） | 细案拍板（T1-T6）后实施 K24-K29 | 备料已做：v1 `abstractCanon` 产出格式/书指纹缓存机制已归档（细案附录 A）；现状=手工 ctx 文本 + 静态张力 `tension=0.5` 占位（pack 槽位已存在） |
| 事件产率上限 | **已实施（K19：EVENT_CAPS.perTick=6 提案）** | —（已办结） | 分布曲线在案（细案 §1：max 4 开局 / 稳态 1）；正式报批随报批支线 |
...
| 玩家档案细案（原"玩家棋子入账"设计缺口） | **已拍板（P-A~P-F 全案）· K8-K12 实施完成（2026-09-07）** | —（已办结） | P-1..P-7 判据全达（128/128）；影响系数/初始 attrs K11 曲线在案、正式报批待拍；玩家 attrs 自动注入触发=设定注入管线（未变） |
...
| 掩码观察者方向（评审发现：实施与拍板句意相悖——P-5"low weight→远事不可见" vs 实施 m=src/obs 低分量更可见；obs→0 断崖） | **已拍板（2026-09-07：保留比值方向）· 修边已入（obsFloor 0.05 提案态，145/145）** | —（已办结） | ε 随掩码系数整体报批（报批支线）；P-5 句意按实施口径理解 |
