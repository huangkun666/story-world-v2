...
- **测试**：`node --test`（**必须无参**——Node 24 下目录参数会被当模块加载）。当前 220/220 全绿（2026-09-08 第十棒实测；player-inject v1.1 5→8 则 + bystander 补跑重放组入面）。
...
| 模型输出快照 fixture | **已实施（K23：转制器 + ×8 fixture + 重放回归；2026-09-08 bystander 补跑入面，缺口①闭合）** | —（已办结） | 局限实录（台账 L58）：夹具补丁前 2 份为历史文物、step-null 后段不可重放；增强建议=快照落盘 world 状态（capture-demo 升级，K23 缺口②——下轮真跑已触发，待用户点单） |
