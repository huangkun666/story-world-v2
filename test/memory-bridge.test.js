...
test('M3：当下表恰好一条（覆盖）；发生表**分档如实**（已了结 / 未结 / 已归卷）', () => {
    ...
    // ★★leg53（口径升级）：民生那一格撤了 ⇒ 这一面（记忆插件里那张卡）也不再报它。
    //   判据：剩下三格各报一次；民生**一个字节都不许出现**（否则就是"面板没有、记忆里有"的两把尺子）。
    assert.ok(st.详细说明.includes('动乱度：动荡 · 天时：未定 · 张力推手：未定'),
        '参数档位照抄原话（未定的照实写未定；三格口径，民生已撤）');
    assert.ok(!st.详细说明.includes('民生'), '★leg53：记忆插件里也不许再报「民生」（面板上没有它）');
