...
test('★leg40b 续 + leg54 + leg63：参数页把世界尺度做成**五个可调输入框**（旧"档位下拉"口径已升级）', () => {
    ...
    // ④ ★★★leg63：**「每轮新生」已经给了旋钮**（用户实机报「我参数都这样了」——
    //   他把「每轮递几条线」拧到 10 仍被 `盘算大厦顶（每 tick 新生 ≤3）` 拒签，
    //   而那一道过去读的是出厂常量、面板上根本没框 ⇒ 他的意图被一个看不到的数吃掉）。
    //   本条判据**反向锁**：它必须**在**（画了框 + 值上板 + 与引擎真源同源）。
    assert.ok(seg.includes('data-param="每轮新生"'), '★leg63：「每轮新生」必须做成可写参数（不再是只读的固定闸）');
    ...
    assert.match(seg, /还有两个数不给拧/, '★leg63：固定清单从三个降到两个（每轮新生已转正）');
});
...
test('★★★leg54：**"无上限"必须写在脸上 + 后果如实说**（填大了会以什么形式表现出来）', () => {
    ...
    // 用户问过"四个框"，现在必须是五个——文案里的数要跟着键表走，不许留旧字面
    assert.match(cap, /五个框都能/, '★"五个框"（leg63 起多一个「每轮新生」）');
...
test('★细案编年页（leg50）：版位升位且不含引擎术语（构建号在玩家视线内）', () => {
    ...
    assert.equal(PANEL_BUILD, 'leg63-scale-index-2');
