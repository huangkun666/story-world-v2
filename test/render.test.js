...
test('★细案编年页（leg50）：版位升位且不含引擎术语（构建号在玩家视线内）', () => {
    ...
    // ★★★leg63 换档（两次）：刻度按原文条目分节 + 表/档折起；随后「每 tick 新生」转正（世界行为也变了）。
    // ★★★leg64 换档：设定页「法则」一栏**按类别分段 + 如实报进包条数**（此前不报进包）⇒ 玩家可见面真变了。
    //   ★起名纪律又咬了一次：第一版 `leg64-rule-kinds` 含禁词 `kind`（就在下面这个循环里）⇒ 改
    //     `leg64-rule-classes` 才过。这条雷 leg50/52 各踩过一次，本棒是第三次——扫描器在位是对的。
    assert.equal(PANEL_BUILD, 'leg64-rule-classes');
