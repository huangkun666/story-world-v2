...
test('★leg60 一块里名册与设定同轮抽（旧"名册轮只问 name/kind / 不许问 powerScale"按新口径改写）', async () => {
    ...
    // ★★★leg64 第二轮改口径（用户令「世界设定和判定依据都要」）：第一版写的"大多数是其他"**已删**——
    //   它把世界观当成了残渣，实机重抽当场证明是错的（世界观升格成一类、也进包）。
    //   现在守的是那条**真正防灌满**的口径：判据是稀疏的（"一本书通常只有二三十条"）。
    assert.ok(p.includes('一本书通常只有'), '★明写"判据是稀疏的（二三十条）"（不这么写，判据那一类会被灌满）');
    assert.ok(p.includes('世界观设定'), '★世界观是**独立一类**（不是"其他"桶里的残渣）');
